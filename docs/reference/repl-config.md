...
|---|---|---|---|
| `prompt` | `Function<Object, String>` | `_ -> "SpEL> "` (root); `null` (script) | Before reading a line, if non-`null`. |
| `isCommentLine` | `BiFunction<Object, String, Boolean>` | `(_, s) -> s.trim().startsWith("//")` | While reading — matching lines are skipped. |
| `expressionInterceptor` | `BiFunction<Object, String, String>` | shorthand rewrite + history logging (wrapped once more by `BaseSpelShellImpl` for backtick expansion) | After a full expression is read; returns what gets evaluated. |
...
| `stopOnException` | `Class<? extends Exception>` | `ShellExitException.class` (root); `Exception.class` (script) | An exception whose class this is assignable from ends `runRepl()` instead of being caught and printed. See [page 10](../tutorial/10-error-handling.md). |
